...
namespace Loader
{
  ...
  void Loader::_prepare()
  {
    ...
    //     return;
    if (_is_flash_prog)
      if (!fs::exists(_flash_file.toStdString()))
        return;

    if (_is_ext_flash_prog)
      if (!fs::exists(_ext_flash_file.toStdString()))
        return;
    if (_is_eeprom_prog)
      if (!fs::exists(_eep_file.toStdString()))
        return;
    ...
    if (_is_ext_to_int)
      _stg_vec.push_back(Stage::EXT_TO_INT);
  ...
  void Loader::_do_flash_prog_step()
  {
    ...
    _flash_page_idx++;
    _cur_step++;
    if (_flash_page_idx == _flash_pages.size())
      _stage++;
  }
  ...
  void Loader::_do_eep_prog_step()
  {
    ...
    _eep_page_idx++;
    _cur_step++;
    if (_eep_page_idx == _eep_pages.size())
      _stage++;
  }
  ...
  int CMD::_put_packet(Command cmd, std::vector<uint8_t> data)
  {
    ...
    int ret = _serial.write(reinterpret_cast<const char *>(raw.data()), raw.size());
  ...
  bool CMD::cmd_v1_prog_end()
  {
    ...
    std::vector<uint8_t> data;
    _get_packet(ret_cmd, data);
    bool &&cmp = (ret_cmd == Command::PROG_END) && (memcmp(data.data(), "OK!!", 4) == 0);
  ...
  bool CMD::cmd_v2_flash_write(uint32_t page_addr, std::vector<uint8_t> data)
  {
    ...
    payload.insert(payload.end(), make_move_iterator(data.cbegin()), make_move_iterator(data.cend()));
    // delete[] payload;
  ...
  std::vector<uint8_t> CMD::cmd_v2_flash_read()
  {
    ...
    return std::vector(make_move_iterator(ret.begin() + 1), make_move_iterator(ret.end()));
  ...
  bool CMD::cmd_v2_ext_flash_write(uint32_t page_addr, std::vector<uint8_t> data)
  {
  ...
  {
    void *pac =
        _advance_cmd(Command::EEPROM_WRITE, page_data).data() + 1;
    if (pac)
